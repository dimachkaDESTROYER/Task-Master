    ...
    public class TelegramTaskBot
    {
        ...
        public async void RecieveKeyButton(CallbackQueryEventArgs args, TelegramBotClient bot)
        {
            ...
            else if (data == "\u21a9\ufe0f")
            {
                ...
                await bot.EditMessageTextAsync(chatId, args.CallbackQuery.Message.MessageId, "Выберите список задач", replyMarkup: keyboard);
            }
                    ...
                    case "Изменить задачу":
                        {
                            ...
                            var keyboard = GetKeyboardListTasks();
                            await bot.SendTextMessageAsync(args.Message.Chat.Id, "Выберите список задач", replyMarkup: keyboard);
                            break;
                        }
